...
// enum JoinStatus { Loading, IsOwner, NotJoined, Pending, Joined, Declined }
...
//   // --- UPDATED: Sửa hàm _onJoinPressed ---
...
//       // (Việc kiểm tra xung đột sẽ diễn ra bên trong dialog)
//       setState(() => _joinStatus = JoinStatus.NotJoined); // Tắt loading
//       _showTeamSelectionDialog(newEventTime);
//     }
//   }

//   // --- NEW: Hàm kiểm tra xung đột lịch ---
//   Future<bool> _checkScheduleConflict(
//     String entityId, // Đây có thể là userId hoặc teamId
//     Timestamp newEventTime,
//   ) async {
//     // Lấy thời gian kết thúc của sự kiện mới
//     final Timestamp? newEventEndTime = _eventData['eventEndTime'];
//     if (newEventEndTime == null) {
//       // Nếu không có thời gian kết thúc, giả sử sự kiện kéo dài 2 giờ
...
//   ) async {
//     try {
//       // --- 1. Kiểm tra các sự kiện entity này đã ĐƯỢC CHẤP NHẬN ---
...
//       // --- 2. Kiểm tra các sự kiện entity này TỔ CHỨC ---
...
//   // --- NEW: Dialog thông báo trùng lịch ---
...
//   // --- UPDATED: Sửa hàm _showTeamSelectionDialog ---
...
//             // ... (code loading và error giữ nguyên) ...
...
//                       title: Text(teamName),
//                       onTap: () async {
//                         Navigator.of(context).pop(); // Đóng bảng
//                         setState(
//                           () => _joinStatus = JoinStatus.Loading,
//                         ); // Bật loading

//                         // --- (KIỂM TRA XUNG ĐỘT CHO TEAM) ---
//                         bool hasConflict = await _checkScheduleConflict(
//                           team.id,
//                           newEventTime,
//                         );
...
//   // --- UPDATED: Sửa _sendJoinRequest (bỏ setState Loading) ---
...
//     // Bỏ dòng: setState(() => _joinStatus = JoinStatus.Loading);
//     // (Vì đã set ở hàm _onJoinPressed hoặc trong dialog)
...
//   // ... (Các hàm _fetchOrganizerDetails, _formatEventTime, _getSportVisual, build, _buildJoinButton, _buildDetailTile giữ nguyên) ...
...
//         _eventData['eventEndTime']; // Thêm thời gian kết thúc
...
//                   color: Colors.white, // Đảm bảo chữ luôn là màu trắng
//                 ),
//               ),
//               background: Stack(
//                 // <-- Bọc ảnh bằng Stack
...
//                           Colors.black.withOpacity(0.7), // Đen mờ ở dưới
//                         ],
//                         stops: const [0.5, 1.0], // Bắt đầu mờ từ 50% chiều cao
//                       ),
//                     ),
//                   ),
//                 ],
...
//                     ),
//                     _buildDetailTile(
//                       Icons.leaderboard_outlined, // Icon cho trình độ
//                       'Trình độ',
//                       skillLevel,
//                     ),
//                     _buildDetailTile(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return ListTile(
                      ...
                      onTap: () async {
                        ...
                        // --- CHECK 2: Kiểm tra xung đột lịch ---
                        bool hasConflict = await _checkScheduleConflict(
                          team.id,
                          newEventTime,
                        );
